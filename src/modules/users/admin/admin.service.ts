...
export class AdminService {
  ...
  async createNewUser(payload: CreateNewUserDto) {
    ...
    try {
      ...
      const resultOfRole = rolesToCheck.every((role) => UserRolesArray.includes(role));
      if (!resultOfRole) { return errorResponse('Invalid Roles!')}
      const email = payload.fullName ? payload.fullName.toLowerCase() + '@gmail.com' : null;
  ...
  async getAdminDashboardInfo(): Promise<ResponseModel> {
    try {
      ...
      const totalBlock = await PrismaClient.block.count();
  ...
  async getAdminDashboardInfoOld(): Promise<ResponseModel> {
    try {
      const totalUsers = await PrismaClient.user.count();
      const totalCourses = await PrismaClient.course.count();
