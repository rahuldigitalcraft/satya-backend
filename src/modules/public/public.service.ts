...
export class PublicService {
  ...
  async getLandingSrtstPageData() {
    try {
      const data: any = await getAdminSettingsData(LandingSRTSTPageSlugs);
      const landingData = {
        landing_data: data,
      };
      return successResponse('Srtst Landing page data!', landingData);
    } catch (error) {
      processException(error);
    }
  }
